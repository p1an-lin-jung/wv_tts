...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        if self.use_spk:
            ...
            _,spk_filename,_ = filename.split('/')
            spekaer_id=self.speaker_map[spk_filename]
            speakers = torch.from_numpy(np.array(spekaer_id)).long()

        if not self.use_sr:
            c_filename = filename.replace(".wav", ".pt")
            c_filename = c_filename.replace("DUMMY", "dataset/wav2vec2_xlsr")
            ...
            c_filename = c_filename.replace("DUMMY", "dataset/sr/wav2vec2_xlsr")
            c = torch.load(c_filename).squeeze(0)
        ...
        if self.use_spk:
            ...
            return c, spec, audio_norm,speakers
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        if self.use_spk:
            ...
            spks = torch.LongTensor(len(batch), 1)

        c_padded = torch.FloatTensor(len(batch), batch[0][0].size(0), max_spec_len)# b,1024,len
        spec_padded = torch.FloatTensor(len(batch), batch[0][1].size(0), max_spec_len)# [b,641,len]
