...
def process(filename):
    ...
    '''
    for i in range(args.min, args.max+1,2):
        ...
        with torch.no_grad():
            c = cmodel(wav_rs, output_hidden_states=True)

        ssl_path = os.path.join(ssl_dir, basename.replace(".wav", f"_{i}.pt"))
        torch.save(c.hidden_states[-1].cpu(), ssl_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ssl_dir", type=str, default="dataset/sr/wav2vec", help="path to output ssl dir")
